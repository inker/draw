import buffersToBits from '#utils/buffersToBits';
...
const take = async (bits: ReturnType<typeof buffersToBits>, count: number) => {
  const taken: number[] = [];
...
describe('buffersToBits', () => {
  ...
  it('passes over a buffer with no bytes in it', async () => {
    const bits = buffersToBits(cycle([], [0b1010_1010]));
  ...
  it.each([1, 3, 32, 64])('takes a buffer of any width: %s', async width => {
    ...
    const bits = buffersToBits(cycle(bytes));
  ...
  it('ends when a finite source runs out', async () => {
    const bits = buffersToBits([new Uint8Array([0b1100_0011]).buffer]);
  ...
  it('takes a sync iterable just as well', async () => {
    ...
    expect(await take(buffersToBits(sync()), 16)).toBe('0000111111110000');
  });
});
