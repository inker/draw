...
export default function (pots: Team[][], matchups: Team[][], currentMatchupNum: number): number[] {
    ...
    function branchHasDescendants(branchNum: number, currentMatchupNum: number): boolean {
        ...
        const hasDescendants = ++currentMatchupNum === matchups.length || simulateDraw(matchups, currentMatchupNum);
        currentMatchup.pop();
        groupWinners.splice(branchNum, 0, o);
        return hasDescendants;
    }

    function simulateDraw(virtualMatchups: Team[][], virtualMatchupNum: number): boolean {
        ...
        return runnersUp.some((ru, ruIndex) => {
            ...
            const hasAnyDescendants = groupWinnersIndices.some(j => branchHasDescendants(j, virtualMatchupNum));
            virtualMatchup.pop();
    ...
    return groupWinners
        .map((team, i) => i)
        .filter(i => branchHasDescendants(i, currentMatchupNum));
}