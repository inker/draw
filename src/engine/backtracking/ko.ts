...
function anyGroupWinners<T>(
  ...
  return (
    nextMatchupNum === newMatchups.length ||
    // eslint-disable-next-line no-use-before-define
    anyRunnersUp(
      [groupWinners.filter(i => i !== item), runnersUp],
      newMatchups,
      predicate,
    )
  )
...
function anyRunnersUp<T>(
  ...
  return groupWinners.some(item =>
    anyGroupWinners(item, [groupWinners, newRunnersUp], newMatchups, predicate),
  )
}
